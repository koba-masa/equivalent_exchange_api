...
FROM
  v_wants AS my_wants
  INNER JOIN v_stocks AS your_stocks ON my_wants.character_id = your_stocks.character_id AND your_stocks.status = 0 AND my_wants.user_id <> your_stocks.user_id
  INNER JOIN v_wants AS your_wants ON your_wants.user_id = your_stocks.user_id AND my_wants.good_id = your_wants.good_id AND your_wants.status = 0
  INNER JOIN v_stocks AS my_stocks ON my_wants.user_id = my_stocks.user_id AND my_wants.good_id = my_stocks.good_id AND your_wants.character_id = my_stocks.character_id AND my_stocks.status = 0
WHERE
  my_wants.status = 0
ORDER BY
    my_user_id
